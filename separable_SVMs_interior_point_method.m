...
while (1)                                     % Outer loop 
       ...
       while ( 1 )                            % Inner loop 
          ...
          if (l_x^2/2 <= threshold_1) 
           ...
           while ( barrier_SVM_cost_function( w_new, X_augm, y, t ) > barrier_SVM_cost_function( w(:,inner_iter), X_augm, y, t ) + alpha * tau * grad' * Dw_Nt )       
                tau = beta * tau; 
                w_new = w(:,inner_iter) + tau * Dw_Nt;
           end
           ...
           figure(2), plot([w_SVM w(:,inner_iter)]), 
